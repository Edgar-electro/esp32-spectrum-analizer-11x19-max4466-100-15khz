...
#define SAMPLING_FREQ   26000     // Hz, must be 40000 or less due to ADC conversion time. Determines maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    if (vReal[i] > NOISE) {                    // Add a crude noise filter
        ...
        //20 bands, 17 kHz  40000 frequenci   256 bit  samples
      ...
      if (i>10  && i<=13 ) bandValues[9]  += (int)vReal[i];
      if (i>13  && i<=17 ) bandValues[10] += (int)vReal[i];
      if (i>17  && i<=23 ) bandValues[11] += (int)vReal[i];
      if (i>23  && i<=29 ) bandValues[12] += (int)vReal[i];
      if (i>29  && i<=38 ) bandValues[13] += (int)vReal[i];
      if (i>38  && i<=50 ) bandValues[14] += (int)vReal[i];
      if (i>50  && i<=66 ) bandValues[15] += (int)vReal[i];
      if (i>66  && i<=86 ) bandValues[16] += (int)vReal[i];
      if (i>86  && i<=112 ) bandValues[17] += (int)vReal[i];
      if (i>112  && i<=147 ) bandValues[18] += (int)vReal[i];
      if (i>192            ) bandValues[19] += (int)vReal[i];
